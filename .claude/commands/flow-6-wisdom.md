...
`gh-issue-manager` -> final update to issue body status board. If the issue is missing and gh is available, it may create it (with a Signal-pending banner when created from Flow 6). Marks run as complete.
